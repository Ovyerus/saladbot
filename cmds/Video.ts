...
export default class Video extends Command {
  ...
  async main(ctx: GuildContext) {
    ...
    if (!channelID)
      return ctx.send("You need to join a voice channel to use this.");

    await ctx.send(
      `Click this link while in **<#${channelID}>** to switch to video: <https://discordapp.com/channels/${ctx.guild.id}/${channelID}>`
    );
  }
}
